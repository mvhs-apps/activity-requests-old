...
router.post('/submit_form', (req, res) => {
  ...
  mailer.sendMail({
    ...
    return mailer.sendMail({
      ...
      messageHTML: formObject.general.student_name + " has submitted an approval for " + formObject.general.club_name + ". Please accept or reject the approval below.<br /><button><a href='localhost:8080/approve?" + formObject.id_num + "?clubAdvisor'>Accept</a></button><button><a href='localhost:8080/reject?" + formObject.id_num + "?clubAdvisor'>Reject</a></button>"});
  }).then((data) => {
    if(formObject.campus.cafeteria === true) {
      return mailer.sendMail({
        ...
        messageHTML: formObject.general.student_name + " has submitted an approval for " + formObject.general.club_name + ". Please accept or reject the approval below.<br /><button><a href='localhost:8080/approve?" + formObject.id_num + "?cafeteria'>Accept</a></button><button><a href='localhost:8080/reject?" + formObject.id_num + "?cafeteria'>Reject</a></button>"});
    }
    return;
  }).then((data) => {
